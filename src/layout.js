  ...
  return layout.pages.map(page => {
    ...
    for (const r of page.rows) {
      ...
      for (const c of page.columns) {
        ...
        cells += `<td data-row="${r}" data-column="${c}" rowspan="${rs}" colspan="${cs}" style="${css}">${bar ? `<span class="print-data-bar" aria-hidden="true" style="width:${bar.ratio * 100}%;background:${safeColor(bar.color, '#21a366')}"></span>` : ''}<span class="print-cell-text">${escapeHTML(cell.Text)}</span></td>`;
      }
