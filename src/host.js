...
export function createHostBridge(control,{postMessage=()=>{}}={}){
 ...
 const methods={
  ...
  'worksheets.add':r=>{const s=control.Workbook.Worksheets.Add(r.name,r.index);return{name:s.Name,id:s.Id};},
  'worksheets.move':r=>{const s=control.Workbook.Worksheets.Move(sheet(r),r.index);return{name:s.Name,id:s.Id,position:control.Workbook._sheets.indexOf(s)};},
  ...
  'range.fillSeries':r=>range(r).FillSeries(r.options??{}),
  'range.pasteSpecial':r=>{if(typeof r.sourceAddress!=='string')throw new TypeError('sourceAddress is required');return range(r).PasteSpecial(range({sheet:r.sourceSheet??r.sheet,address:r.sourceAddress}),r.options??{}).Address;},
  'range.specialCells':r=>range(r).SpecialCells(r.type,r.options??{}).map(cell=>cell.Address),
