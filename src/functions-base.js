...
export function createFunctionRegistry() {
  ...
  add('INDEX', (a, row, col = 1) => { a = matrix(a); row = Math.trunc(number(row)); col = Math.trunc(number(col)); if (row < 0 || col < 0 || row > a.length || col > a[0].length) throw error('#REF!'); if (!row) return col ? a.map(r => [r[col - 1]]) : a; if (!col) return [a[row - 1]]; return a[row - 1][col - 1]; });
