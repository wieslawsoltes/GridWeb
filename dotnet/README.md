...
## Scoped-name commands (0.8)

`DefineScopedNameAsync`, `ListDefinedNamesAsync`, `GetDefinedNameAsync`, `RenameDefinedNameAsync` and `RemoveDefinedNameAsync` extend SpreadsheetClient. A worksheet name/ID selects local scope; null selects the workbook. DefinedNameOptions supplies comment, hidden, contextSheetId and baseAddress; null options are omitted. Cancellation and rejected local arguments do not invoke transport. Generic InvokeAsync also exposes `names.createFromSelection` (source sheet/address, optional scopeSheet and boundary flags). Native smoke includes six name/binding/persistence assertions. The separately pinned Blazor package is not advanced by these root changes.
